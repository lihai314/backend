...
LOCAL_CHECK_PROJECT ?= backend-local-check
LOCAL_CHECK_BACKEND_PORT ?= 18000
LOCAL_CHECK_POSTGRES_PORT ?= 15432
LOCAL_CHECK_REDIS_PORT ?= 16379

export PYTHON_BASE_IMAGE
export UV_BASE_IMAGE

.PHONY: help install hooks-install hooks-run format lint lint-fix type-check test test-cov check docker-build docker-up docker-down docker-ps docker-logs docker-health docker-local-check
...
help:
	...
	@echo "  make docker-up     构建并启动后端、PostgreSQL 和 Redis"
	@echo "  make docker-down   停止并删除本地 Docker 容器和网络，不删除数据卷"
	@echo "  make docker-ps     查看本地 Docker 服务状态"
	@echo "  make docker-logs   查看后端容器日志"
	@echo "  make docker-health 检查后端健康接口"
	@echo "  make docker-local-check 使用隔离项目构建、联调、测试并清理临时 Docker 资源"
...
docker-local-check:
	...
	uv_image_exists=0; \
	compose_env="COMPOSE_PROJECT_NAME=$(LOCAL_CHECK_PROJECT) BACKEND_IMAGE=backend:local-check BACKEND_PORT=$(LOCAL_CHECK_BACKEND_PORT) POSTGRES_PORT=$(LOCAL_CHECK_POSTGRES_PORT) REDIS_PORT=$(LOCAL_CHECK_REDIS_PORT) BACKEND_CONTAINER_NAME=$(LOCAL_CHECK_PROJECT)-api POSTGRES_CONTAINER_NAME=$(LOCAL_CHECK_PROJECT)-postgres REDIS_CONTAINER_NAME=$(LOCAL_CHECK_PROJECT)-redis"; \
	...
	cleanup() { \
		status=$$?; \
		env $$compose_env docker compose down -v --remove-orphans >/dev/null 2>&1 || true; \
	...
	attempt=1; \
	until env $$compose_env docker compose up -d --build; do \
	...
	i=0; \
	until curl -fs http://127.0.0.1:$(LOCAL_CHECK_BACKEND_PORT)/api/v1/health >/dev/null; do \
		i=$$((i + 1)); \
		if [ "$$i" -ge 30 ]; then echo "后端健康检查超时"; exit 1; fi; \
		sleep 1; \
	done; \
	curl -fsSI http://127.0.0.1:$(LOCAL_CHECK_BACKEND_PORT)/docs >/dev/null; \
	env $$compose_env docker compose exec -T redis redis-cli ping >/dev/null; \
	env $$compose_env docker compose exec -T postgres pg_isready -U postgres -d backend >/dev/null; \
	env $$compose_env docker compose config >/dev/null; \
	uv run mypy src; \
	uv run pytest
